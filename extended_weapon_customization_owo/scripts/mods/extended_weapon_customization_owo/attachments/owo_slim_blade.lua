...
function mod.owo_slim_blade(given_slot_name, given_attachment_node)
    ...
    local function slim_blade_attach_helper(number_string, name_to_use, base_item_address, scales_table)
        ...
        create_an_attachment(table_to_return, slim_blade_name,
            ...
            {   replacement_path = _item_melee.."/blades/"..slim_blade_name,
                ...
                custom_selection_group = attachment_group_name..name_to_use,
            },
            -- Fixes
            fixes_to_add,
            -- Kitbash
    ...
    for_all_weapon_models(2, nil, function(number_string)
        slim_blade_attach_helper(number_string, "flat_pfalchion_", _item_melee.."/blades/power_falchion_blade_", flat_pfalchion_scl)
    end)
    -- Flat pfalchion (Grip)
    for_all_weapon_models(2, nil, function(number_string)
        slim_blade_attach_helper(number_string, "flat_pfalchion_g_", _item_melee.."/blades/power_falchion_blade_", {flat_pfalchion_g_scl, grip_scale})
    end)
    -- Slim pfalchion
    for_all_weapon_models(2, nil, function(number_string)
        slim_blade_attach_helper(number_string, "slim_pfalchion_", _item_melee.."/blades/power_falchion_blade_", slim_pfalchion_scl)
    end)
    -- Slim pfalchion (Grip)
    for_all_weapon_models(2, nil, function(number_string)
        slim_blade_attach_helper(number_string, "slim_pfalchion_g_", _item_melee.."/blades/power_falchion_blade_", {slim_pfalchion_g_scl, grip_scale})
    end)
    -- ------------------
    -- "Devil's Claw" Blades
    -- ------------------
